...
//                 document.querySelector("filledWishlist.d-none").classList.remove("d-none");
//             }
...
//         const emptyWishlistSection = document.querySelector("emptyWishlist.d-none");
...
document.addEventListener("DOMContentLoaded", async () => {
            ...
            <div class="col-12 col-sm-6 col-lg-4 col-xxl-3 mb-5 mb-sm-4 wishlist-item" data-id="${item.id}">
                <div class="V_border mx-auto h-100 d-flex flex-column pb-3">
                    ...
                    <div class="V_name_width mx-auto">
                        ...
                        <div class="d-flex V_height justify-content-center mt-2 color-options" id="sliderContainer-${item.id}">
                            ${colorOptionsHTML}
                            ${moreOptionsHTML}
                        </div>
    ...
    async function renderWishlist() {
        ...
        const wishlistData = userData.wishlist || [];
        const wishlistContainer = document.getElementById("wishlistContainer");
        const emptyWishlistSection = document.querySelector(".emptyWishlist");
        const filledWishlistSection = document.querySelector(".filledWishlist");

        if (wishlistData.length > 0) {
            wishlistContainer.innerHTML = wishlistData.map(item => createWishlistItemHTML(item)).join("");
            emptyWishlistSection.classList.add("d-none");
            filledWishlistSection.classList.remove("d-none");
        } else {
            wishlistContainer.innerHTML = "";
            emptyWishlistSection.classList.remove("d-none");
            filledWishlistSection.classList.add("d-none");
        }
        
        applyColorOrImageSelection();
    }

    // Apply color selection to items
    function applyColorOrImageSelection() {
        document.querySelectorAll(".wishlist-item").forEach(item => {
            const itemId = item.getAttribute("data-id");
            const storedColor = localStorage.getItem(`selectedColor_${itemId}`);
            
            if (storedColor) {
                const colorElement = item.querySelector(`.V_color_border[data-color="${storedColor}"]`);
                if (colorElement) {
    ...
    // Event listener for color/image selection
    document.getElementById("wishlistContainer").addEventListener("click", async event => {
        const selectedElement = event.target.closest(".V_color_border");
        if (!selectedElement) return;
        ...
        // Reset borders for all color options of this item
        document.querySelectorAll(`.V_color_border[data-item-id="${itemId}"]`).forEach(el => {
            el.style.border = "0.6px solid rgba(20, 20, 20, 0.2)";
        });

        // Apply border to selected color
        selectedElement.style.border = "1px solid black";

        // Store selection in localStorage
        localStorage.setItem(`selectedColor_${itemId}`, selectedColor);
    ...
    document.getElementById("wishlistContainer").addEventListener("click", async event => {
        ...
        try {
            ...
            // Show empty wishlist message if necessary
            if (updatedWishlist.length === 0) {
                document.querySelector(".emptyWishlist").classList.remove("d-none");
                document.querySelector(".filledWishlist").classList.add("d-none");
            }
        } catch (error) {
            console.error("Error removing item from wishlist:", error);
        }
    ...
    // Event listener for moving item to cart
    document.getElementById("wishlistContainer").addEventListener("click", async event => {
        const moveToCartButton = event.target.closest(".move-to-cart, .V_cart_btn");
        if (!moveToCartButton) return;

        const itemId = moveToCartButton.getAttribute("data-id");
        const wishlistItemElement = document.querySelector(`.wishlist-item[data-id="${itemId}"]`);
        if (!wishlistItemElement) return;

        try {
            const userData = await fetchUserData();
            const wishlistData = userData.wishlist || [];
            let cartData = userData.orders || [];

            // Find the item in the wishlist
            const wishlistItem = wishlistData.find(item => item.id == itemId);
            if (!wishlistItem) return;

            // Check if item already exists in cart
            const existingCartItem = cartData.find(item => item.id == itemId);
            if (existingCartItem) {
                existingCartItem.quantity += 1;
            } else {
                cartData.push({
    ...
    function initializeSliders() {
        document.querySelectorAll("[id^='sliderContainer-']").forEach(slider => {
            // Add slider functionality if needed
            // This would replace your existing slider code
        });
    }
